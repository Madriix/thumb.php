...
/* 
	...
	Copyright: 
		...
		From version thumb.php v1.2 onwards, this code has been corrected by Mickael B. to ensure it remains functional on PHP 8.2 and later versions.
	______________________________________________________________________
	Configuration:
	...
	Changes:
		...
		1.2 - works on PHP 8.2 and supports the webp image format
*/
...
if (isset($_GET["quality"]) && $_GET["quality"]<>0) {
...
if (isset($_GET["sizex"]) && intval($_GET["sizex"])>0)
{
	$thumb_size_x=intval($_GET["sizex"]);
	if (isset($_GET["sizey"]) && intval($_GET["sizey"])>0) 
	{
...
if (isset($cache_file) && (file_exists($cache_file)) && (@filemtime($cache_file)>@filemtime($filename)))
{
...
{
	case 18: # WEBP
	if (!$image = @imagecreatefromwebp($filename))
	{
		# not a valid gif file
		$image = imagecreatefromwebp($image_error);
		$file_type="webp";			
		if (file_exists($cache_file))
		{
			# remove the cached thumbnail
...
if (isset($thumb_size_x) && $thumb_size_x>0)
{
...
switch ($image_type)
{
	...
	if ( $fileextension == "webp" ) 
	{
		...
		header('Content-Disposition: inline; filename='.str_replace('/','',md5($filename.$thumb_size.$thumb_size_x.$thumb_size_y.$quality).'.webp'));
		@imagewebp($thumbnail,$cache_file);
		imagewebp($thumbnail);
	}else {
		header('Content-type: image/webp');
		header('Content-Disposition: inline; filename='.str_replace('/','',md5($filename.$thumb_size.$thumb_size_x.$thumb_size_y.$quality).'.webp'));
